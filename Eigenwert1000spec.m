...
for es = 0 : 1 : (durchgaenge-1)
  sprintf('Durchgangnummer: %i von %i',es+1,durchgaenge)
	fflush(stdout)
...
b = 250 % Blockgroesse (Teilmatrizengroesse) MUSS MIT n KOMPATIEBEL SEIN
...
zeitTeilSchluss = zeitTeilGesamt - zeitTeilTeil
  ...
  if(schritteTeilTot == maxSchlaufen)
	...
	if q == 0
	maxSchlaufenTeil = 5000
	elseif q == 1
	maxSchlaufenTeil = 2500
	elseif q == 2
	maxSchlaufenTeil = 1000
	elseif q == 3
	maxSchlaufenTeil  = 500
	elseif q == 4
	maxSchlaufenTeil = 100
	elseif q == 5
	maxSchlaufenTeil = 50
	else
	maxSchlaufenTeil = 10
...
csvwrite("WerteDurchschnitt1000spec.csv", Werte)
csvwrite("TeilmatrixGroesse1000spec.csv", WerteDurchschnitt(1:8, 1))
csvwrite("ZeitTeil1000spec.csv", WerteDurchschnitt(1:8, 2))
csvwrite("ZeitOriginal1000spec.csv", WerteDurchschnitt(1:8, 3))